<?php
$menu_item_page = "bank";
$menu_item_second = "add_credit_card";
?>
...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">Add Credit Card</h3>
        ...
        <div class="row">
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <form class="forms-sample" method="POST" action="{{ route('store_credit_card') }}">
                            @csrf
                            <div class="form-group">
                                <label for="">Code</label>
                                <input type="text" class="form-control @if($errors->has('code')) is-invalid @endif" name="code" placeholder="credit card code" value="{{old('code')}}" required/>
                                @if($errors->has('code'))
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @if($errors->has('name')) is-invalid @endif" name="name" placeholder="credit card name" value="{{old('name')}}" required/>
                                @if($errors->has('name'))
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @if($errors->has('bank_account_id')) is-invalid @endif" name="bank_account_id" id="selectBankAccount" required>
                                    ...
                                    @foreach($bankAccounts as $bankAccount)
                                        <option value="{{$bankAccount->id}}" {{ old('bank_account_id') == '$bankAccount->id' ? 'selected' : '' }}>{{$bankAccount->code}} - {{$bankAccount->name}} ( {{$bankAccount->account_number}} )</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control @if($errors->has('cicilan')) is-invalid @endif" name="cicilan" min=0 value="{{old('cicilan')}}" required/>
                            ...
                            <button class="btn btn-light" type="button" onclick='location.href="{{ route('list_credit_card') }}"'>Cancel</button>
                        </form>
                    </div>
                </div>
            </div>
