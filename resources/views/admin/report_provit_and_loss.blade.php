...
<div class="main-panel">
	<div class="content-wrapper">
		<div class="page-header">
  			<h3 class="page-title">Report Provit & Loss</h3>
  			<nav aria-label="breadcrumb">
    			<ol class="breadcrumb">
      				...
      				<li class="breadcrumb-item active" aria-current="page">Report Provit & Loss</li>
    			</ol>
  			</nav>
		</div>
          ...
          <div class="col-xs-6 col-sm-6" style="padding: 0;display: inline-block;">
            ...
            <button id="btn-filter" type="button" class="btn btn-gradient-primary m-1" name="filter" value="-"><span class="mdi mdi-filter"></span> Apply Filter</button>
  			...
  			<div class="col-12 grid-margin stretch-card">
    			<div class="card">
      				<div class="card-body">
      					<h5 style="margin-bottom: 0.5em;">Total : {{ $stockOutOrder->count() }} data</h5>
                        ...
                        @endphp
                          ...
                          @php
                            $subTotal = $perStock['quantity'] * $perStock['in_price'];
                            // $subTotal = $perStock['quantity'] * rand(5000000, 15000000); //testing
                            $totalPriceOut += $subTotal;
                          @endphp
                          <tr>
                          	<td>{{$key+1}}</td>
                              <td>{{$perStock['code']}}</td>
                              <td>{{$perStock['name']}}</td>
        											<td class="text-right">Rp. {{ number_format($perStock['in_price']) }}</td>
                              <td class="text-right">{{$perStock['quantity']}} pcs</td>
                      ...
                      <tfoot>
                        <tr>
                          <th class="fwb text-right bg-light" colspan="5">Total Out Price</th>
                          <th class="bg-light text-right">Rp. {{ number_format($totalPriceOut) }}</th>
                        </tr>
                        <tr>
                          <th class="fwb text-right bg-light" colspan="5">Total Sale Netto</th>
                          <th class="bg-light text-right">Rp. {{ number_format($total_saleNya) }}</th>
                        </tr>
                        <tr>
                          @php
                            $result = $total_saleNya - $totalPriceOut;
                            $styleClass = "bg-success";
                            $spanProvitLoss = "Provit";
                            if($result <= 0){
                              $styleClass = "bg-danger";
                              $spanProvitLoss = "Loss";
                            }
                          @endphp
                          <th class="fwb text-right {{ $styleClass }}" colspan="5">Result : {{ $spanProvitLoss }}</th>
                          <th class="fwb text-right {{ $styleClass }}">Rp. {{ number_format($result) }}</th>
                        </tr>
                      </tfoot>
    							</table>
        				</div>
