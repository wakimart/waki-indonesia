...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">Edit Public Homecare</h3>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control"
                                ...
                                form="add-puhc"
                                onchange="setProduct()"
                                {{ $publichomecare['status'] == "new" ? 'required' : 'disabled' }}>
                                <option disabled selected>
                                    Select Branch
                                </option>
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        @if ($publichomecare->branch_id == $branch->id) selected @endif>
                        ...
                        <div class="form-group">
                            ...
                            <select  class="form-control"
                                ...
                                @foreach ($csos as $cso)
                                    <option value="{{ $cso->id }}"
                                        @if($publichomecare->cso_id == $cso->id) selected @endif>
                                        {{ $cso->code }} - {{ $cso->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select  class="form-control"
                                ...
                                @foreach ($csos as $cso)
                                    <option value="{{ $cso->id }}"
                                        @if($publichomecare->cso_optional_id == $cso->id) selected @endif>
                                        {{ $cso->code }} - {{ $cso->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form id="add-puhc"
                            ...
                            action="{{ route("update_public_homecare") }}">
                            @csrf
                            <input type="hidden"
                                name="id"
                            ...
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="date"
                                        ...
                                        name="start_date"
                                        id="start_date"
                                        value="{{ date('Y-m-d', strtotime($publichomecare['start_date'])) }}"
                                        {{ $publichomecare['status'] == "new" ? 'required' : 'disabled' }}/>
                                </div>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="date"
                                        ...
                                        id="end_date"
                                        onchange="setProduct()"
                                        value="{{ date('Y-m-d', strtotime($publichomecare['end_date'])) }}"
                                        {{ $publichomecare['status'] == "new" ? 'required' : 'disabled' }}/>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control"
                                    ...
                                    placeholder="Address"
                                    rows="3"
                                    required>{{ $publichomecare['address'] }}</textarea>
                            </div>
                                ...
                                <select class="form-control"
                                    ...
                                    @foreach ($provinces as $province)
                                        <option value="{{ $province->id }}"
                                            @if ($publichomecare->province_id == $province->id) selected @endif>
                                            {{ $province->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control"
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            @if ($publichomecare->city_id == $city->id) selected @endif>
                                            {{ $city->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control"
                                    ...
                                    @foreach ($subdistricts as $subdistrict)
                                        <option value="{{ $subdistrict->id }}"
                                            @if ($publichomecare->district_id == $subdistrict->id) selected @endif>
                                            {{ $subdistrict->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <img src="{{ asset("sources/puhc/" . $publichomecare->approval_letter) }}"
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row" id="product_parent_{{ $product_idx }}">
                            ...
                            <input type="hidden" form="add-puhc" name="old_product_id_{{ $product_idx }}" value="{{ $publichomecareproduct->id }}">
                            <div class="form-group col-md-10">
                                ...
                                <select class="form-control ph_product_id"
                                    ...
                                    {{ $publichomecare['status'] == "new" || Gate::check('change-status-product-publichomecare') ? 'required' : 'disabled' }} >
                                    <option disabled selected>
                                        Select Product (Please select branch & schedule first)
                                    </option>
                                    @foreach ($phcProducts as $phcProduct)
                                        <option value="{{ $phcProduct['id'] }}"
                                            @if($publichomecareproduct->ph_product_id == $phcProduct['id']) selected @endif>
                            ...
                            @if($publichomecare['status'] == "new" || Gate::check('change-status-product-publichomecare'))
                                @if ($product_idx == 0)
                                ...
                                <button style="position: absolute; bottom: 0; background-color: red;" class="hapus_product"
                                    value="{{ $product_idx }}"
                                    data-id="{{ $publichomecareproduct['id'] }}">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control"
                                ...
                                id="other_product" 
                                name="other_product" 
                                rows="5" required>{{ $publichomecare['other_product'] }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/js/select2.min.js"
    ...
    @for($i=0; $i<=$product_idx; $i++)
    $("#ph_product_id_{{ $i }}").select2();
    @endfor
    $("#province_id").select2();
    $("#city_id").select2();
...
var product_idx = parseInt("{{ $product_idx }}");
...
$(document).on('click', '.hapus_product', function() {
    if ($(this).data('id')) {
        $("#add-puhc").append(`<input type="hidden" name="delete_product_id[]" value="`+$(this).data('id')+`">`);
    }
