<?php
$menu_item_page = "bank";
$menu_item_second = "list_credit_card";
?>
...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">List Credit Card</h3>
                            ...
                            <table class="table table-bordered" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Bank Account</th>
                                        <th>Cicilan</th>
                                        <th>Charge Percentage Sales</th>
                                        <th>Charge Percentage Company</th>
                                ...
                                <tbody>
                                    @foreach ($datas as $key => $creditCard)
                                        <tr>
                                            <td>{{$key + 1}}</td>
                                            <td>{{ $creditCard->code }}</td>
                                            <td>{{ $creditCard->name }}</td>
                                            <td>{{ $creditCard->bank_account_name }}</td>
                                            <td>{{ $creditCard->cicilan }}</td>
                                            <td>{{ $creditCard->charge_percentage_sales }}</td>
                                            <td>{{ $creditCard->charge_percentage_company }}</td>
                                            <td>{{ $creditCard->estimate_transfer }}</td>
                                            <td>{{ $creditCard->description }}</td>
                                            @can('edit-bank')
                                            <td class="center">
                                                <a href="{{ route('edit_credit_card', $creditCard->id) }}">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form id="frmDelete"
                        method="post"
                        action="{{ route("delete_credit_card") }}">
                        @csrf
...
$(document).ready(function (e) {
    $("#btn-filter").click(function (e) {
        ...
        window.location.href = "{{route('list_credit_card')}}" + urlParamStr;
    });
});
